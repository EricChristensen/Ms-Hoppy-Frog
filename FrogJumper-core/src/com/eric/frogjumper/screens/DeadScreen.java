...
public class DeadScreen implements Screen {
	...
	String leaderBoard = "CgkIiJLhl_IDEAIQDw";
	Stage stage;
	Label label;
	...
	public DeadScreen(Game1 game, float score, PlayScreen oldScreen, Sound smallApplause, Sound largeApplause) {
		this.game = game;
			...
			if (highScores.getItems().size() > 0){
				if (highScores.getItems().get(0).getScore() < this.newScore) {
					...
					if (game.actionResolver.getSignedInGPGS()) {
						game.actionResolver.submitScoreGPGS((int)this.newScore);
	...
	private void stageStuff() {
		...
		label = new Label("Highest Score: Arcade Mode", label_style);
